...
<div class="row"><div class="col-md-12">* All fields marked with asterisk are mandatory</div></div>
    <?php $form = ActiveForm::begin($form_params); ?>
        ...
        <?= $form->field($storage, 'description')->textarea(['rows'=>6]) ?>
        ...
        <?= $form->field($storage, 'type')->dropDownList($types)->label('Volume type') ?>
        <?= $form->field($storage, 'vm_type')->dropDownList($vm_types)->label('I want to use this volume for:') ?>
        <?= $form->field($storage, 'storage')-> label($storage_label) ?>
        <span class="hidden num_of_volumes_dropdown"><?= $form->field($storage, 'num_of_volumes')->dropDownList($multiple) ?></span>
