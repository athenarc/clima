	...
	<table class="table table-striped">
		<tbody>
			<tr>
				...
				<td class="col-md-6 text-left" scope="col">
				<?= ProjectValueDisplay::image($details->image, 'image', $requestHistory); ?>
			</tr>
			<tr>
				<th class="col-md-6 text-right" scope="col">CPU cores per server:</th>
				<td class="col-md-6 text-left" scope="col">
                    <?= ProjectValueDisplay::simpleValue($details->cores, 'cores', $requestHistory); ?>
                    (used <?=round($usage['cpu']/1000,2)?> cores/server οn average)</td>
			</tr>
			<tr>
				<th class="col-md-6 text-right" scope="col">RAM per server:</th>
				<td class="col-md-6 text-left" scope="col">
                    <?= ProjectValueDisplay::simpleValue($details->ram, 'ram', $requestHistory); ?>
                    GBs (used <?=round($usage['ram'],2)?> GBs/server οn average)</td>
			</tr>
		</tbody>
	</table>
</div>
