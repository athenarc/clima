...
/**
 * This is the model class for table "storage_request".
 *
 * @property int $id
 ...
 */
class StorageRequest extends \yii\db\ActiveRecord
{
    private $limits;
    private $role;
    ...
    public function init()
    {
        ...
        $this->limits=StorageLimits::find()->where(['user_type'=>$this->role])->one();
    ...
    public function uploadNew($requestId)
    {
        ...
        Yii::$app->db->createCommand()->insert('storage_request', [
        ...
        if (($this->storage<=$row['storage']) && $autoaccept_allowed)
        {
            ...
            $storage_request=StorageRequest::find()->where(['request_id'=>$project->latest_project_request_id])->one();
    ...
    public static function getActiveProjects()
    {
        ...
        $results=$query->select(['p.id','c.vm_type','p.name', 'c.num_of_volumes as vnum'])
                          ...
                          ->innerJoin('storage_request as c','c.request_id=pr.id')
                          ->where(['>=', 'end_date', $date])
